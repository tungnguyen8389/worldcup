...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['action']) && $_POST['action'] === 'save_settings') {
    ...
    $party_fund_per_loss = (int)($_POST['party_fund_per_loss'] ?? 20000);
    $party_fund_target = (int)($_POST['party_fund_target'] ?? 3000000);
    ...
    try {
        ...
        save_setting('party_fund_per_loss', $party_fund_per_loss);
...
$party_fund_per_loss = (int)get_setting('party_fund_per_loss', 20000);
$party_fund_target = (int)get_setting('party_fund_target', 3000000);
...
<div class="card">
    ...
    <form method="POST">
        ...
        <div class="grid-2col">
            <div class="form-group">
                <label for="party_fund_per_loss">Số Tiền Cho 1 Điểm Bị Trừ (VNĐ)</label>
                <input type="number" name="party_fund_per_loss" id="party_fund_per_loss" class="form-control" value="<?php echo htmlspecialchars($party_fund_per_loss); ?>" required>
            </div>
            <div class="form-group">
        ...
        <div class="admin-meta-info" style="margin-top: 15px; background: rgba(0, 204, 119, 0.05); border-left: 3px solid var(--accent); padding: 12px; border-radius: 4px; font-size: 13.5px; color: var(--text-muted); margin-bottom: 20px;">
            <i class="fa-solid fa-circle-info" style="color: var(--accent);"></i> <strong>Cách tính tự động:</strong> Tổng quỹ liên hoan và danh sách Mạnh Thường Quân đóng góp nồi lẩu sẽ được tự động tính toán dựa trên <strong>số điểm bị trừ (số trận đoán sai) của từng người chơi nhân với số tiền tương ứng cho mỗi điểm</strong>, xếp hạng từ cao xuống thấp.
        </div>
